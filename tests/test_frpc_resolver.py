...
@respx.mock
def test_resolve_dim_link_success():
...
@respx.mock
def test_resolve_dim_link_invalid_dim_length():
    with httpx.Client() as client:
...
@respx.mock
def test_resolve_dim_link_http_error():
    respx.post("https://mapy.com/api/mapybox-ng/").mock(
...
@respx.mock
def test_resolve_dim_link_no_geometries():
    mock_payload = bytes(
        pyfrpc.encode(
